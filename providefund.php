...
if (isset($_POST["fund"])){
    ...
    if ($amount > $balance) {
        ...
        $sql6 = "SELECT * FROM oca WHERE OCA_ID = $oid";
        $rows6 = mysqli_query($conn, $sql6);
        $row6 = mysqli_fetch_assoc($rows6);
        $_SESSION["balance"] = $row6["Fund_Balance"];
        $_SESSION["fund"] = $row6["Fundings"];
    ...
    $sql13 = "SELECT * FROM oca WHERE OCA_ID = $oid";
    $rows13 = mysqli_query($conn, $sql13);
    $row13 = mysqli_fetch_assoc($rows13);
    $_SESSION["balance"] = $row13["Fund_Balance"];
    $_SESSION["fund"] = $row13["Fundings"];
