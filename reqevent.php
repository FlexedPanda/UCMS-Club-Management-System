...
if (mysqli_num_rows($chkrows) == 0) {
    if ($entry_fee >= 0) {
        ...
        header("Location: arrange.php?error=Enter Valid Entry Fee");
        exit();
    }
} else {
    header("Location: arrange.php?error=Requested Event Already Exists");
    exit();
}
header("Location: arrange.php?msg=Event Request Has Been Sent");
exit();
?>