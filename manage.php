...
<body>
  <?php
    $club = $_SESSION["club"];
    $sql1 = "SELECT * FROM unapproved_event e, request_event r WHERE e.Event_ID = r.Event_ID AND r.Approved = 'Yes' AND r.Proposed_Club = '$club'";
    ...
    <div class="row mt-5 ml-2 mr-2">
      <?php while($row1 = mysqli_fetch_assoc($rows1)) {?>
        <?php 
          ...
          $oid = $row1["OCA_ID"];
          $sql3 = "SELECT * FROM oca WHERE OCA_ID = '$oid'";
          $rows3 = mysqli_query($conn, $sql3);
          $row3 = mysqli_fetch_assoc($rows3); ?>
        <div class="col-md-4 my-3">
          <div class="card" style="border-radius: .5rem;">
            <div class="card-body">
              <center><img src="misc/logo.png"></center>
              <h6 class="card-title text-center text-dark"><?php echo $row2["Name"]; ?></h6>
              ...
              <h6 class="card-text m-2 pl-4 text-dark"><b>Approved By : </b><?php echo $row3["Name"]; ?></h6><hr>
              <?php if (!isset($_POST["manage"]) || $_POST["manage"] != $row1["Event_ID"]) { ?>
                <form class="form-inline m-2 pl-4" action="manage.php?warn=Providing Event Details" method="POST">
                  <button class="btn btn-primary mt-2" name="manage" value="<?php echo $row1["Event_ID"]; ?>">Manage</button>
                  <button class="btn btn-danger mt-2 ml-3" formaction="organize.php" name="postpone" value="<?php echo $row1["Event_ID"]; ?>">Postpone</button>
                </form>
              <?php } else { ?>
                <form action="organize.php" method="POST">
                  ...
                  <div class="form-group row px-4">
                    <div class="col-sm-10">
                      ...
                      <a class="btn btn-danger mt-2 ml-2" href="manage.php?error=Event Details Cancelled">Return</a>
                    </div>
                  </div>
                </form>
              <?php } ?>
