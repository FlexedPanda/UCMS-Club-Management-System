...
<body>
  <?php
    $id = $_SESSION["id"];
    $sql1 = "SELECT * FROM unapproved_event e, request_event r WHERE r.Event_ID = e.Event_ID AND r.OCA_ID = $id";
    $rows1 = mysqli_query($conn, $sql1); ?>
  <div class="container">
    <div class="row mt-3 ml-2 mr-2">
      <?php while($row1 = mysqli_fetch_assoc($rows1)) { ?>
        <?php
        $club = $row1["Proposed_Club"];
        $sql2 = "SELECT p.Name AS Panel, p.Designation AS Designation, c.Advisor AS Advisor FROM request_event r, registered_member p, club c WHERE r.Member_ID = p.Member_ID AND r.Proposed_Club = c.Name AND c.Name = '$club'";
        ...
        <div class="col-md-4 p-2">
          <div class="card text-light bg-dark" style="border-radius: .5rem;">
            <div class="card-body">
              ...
              <p class="card-text m-2 pl-2"><b>Advisor : </b><?php echo $row2["Advisor"]; ?></p><hr>
              <p class="card-text m-2 pl-2"><b>Requested By : </b><?php echo $row2["Panel"]; ?></p>
              <p class="card-text m-2 pl-2"><b>Designation : </b><?php echo $row2["Designation"]; ?></p><hr>
              <?php if($row1["Approved"] == "No") { ?>
                <form class="form-inline m-2 pl-2" action="arrangement.php" method="POST">
