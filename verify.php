...
if (mysqli_num_rows($rows1) != 0 || mysqli_num_rows($rows2) != 0) {
    header("Location: apply.php?warn=You've Already Sent Request");
    exit();
}
...
//Confirm Check
if ($password != $confirm) {
    header("Location: apply.php?warn=Both Passwords Don't Match");
    exit();
}
...
$sql8 = "SELECT * FROM approved_event e, registered_member p WHERE e.Club = p.Club AND p.Designation <> 'Member' AND e.Event_ID = $event";
